    ...
    def post(self, request, format=None):
        ...
        try:
            if not Product.objects.filter(id=product_id).exists():
                return Response({'error': 'This product does not exits'}, status=status.HTTP_404_NOT_FOUND)
                ...
                if CartItem.objects.filter(cart=cart, product=product).exists():
                    ...
                    if WishListItem.objects.filter(wishlist=wishlist).exists():
                        ...
                        if not WishListItem.objects.filter(wishlist=wishlist).exists():
                            # Update total_items in wishlist
            ...
            return Response({'error': 'Something went wrong when adding iten to cart'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
#         except:
#             return Response({'error': 'Something went wrong when getting total number of items'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def put(self, request, format=None):
        ...
        try:
            ...
            for cart_item in cart_items:
                ...
                if not Product.objects.filter(id=product_id).exists():
                    return Response({'error': 'Product with this ID does not exists'}, status=status.HTTP_404_NOT_FOUND)
