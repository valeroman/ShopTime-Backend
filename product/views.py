    ...
    def post(self, request, format=None):
        ...
        if len(search) == 0:
            ...
            search_results = Product.objects.order_by('-date_created').all()
             ...
             # If there was search criteria, filter by description as well as name containig search data
        ...
        # Check wheter the category exists
        if not Category.objects.filter(id=category_id).exists():
            return Response({'error': 'Category not found'}, status=status.HTTP_404_NOT_FOUND)

        category = Category.objects.get(id=category_id)

        # If category has a parent, filter only by this category and not the parent s well
        if category.parent:
            search_results = search_results.order_by(
                '-date_created'
            ).filter(category=category)
        # If category does no have a parent category, meaning that it itself is  a parent category
        else:
            # If this parent category does not have any children categories 
            # then just filter by category itself
...
class ListRelatedView(APIView):
    ...
    def get(self, request, productId, format=None):
        ...
        # Does a product with this ID exists
        ...
        if Product.objects.filter(category=category).exists():
            ...
            return Response({'error': 'No related product found'}, status=status.HTTP_200_OK)
...
class ListBySearchView(APIView):
    ...
    def post(self, request, fromat=None):
        ...
        # Serializer the product results
        ...
        if len(product_results.data) > 0:
            ...
            return Response({'erro': 'No products found'}, status=status.HTTP_200_OK)
