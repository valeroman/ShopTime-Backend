    ...
    def post(self, request, format=None):
        try:
            ...
            if not CartItem.objects.filter(cart=cart).exists():
                return Response({'error': 'Need to have items in the cart'}, status=status.HTTP_404_NOT_FOUND)
            ...
            if PaymentIntent.objects.filter(user=user).exists():
                ...
                if intent['status'] == 'succeeded':
                    ...
                    return Response({'clientSecret': intent['clientSecret']}, status=status.HTTP_200_OK)
            ...
            return Response({'error': 'Something went wrong when creating stripe payment intent'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
