...
class CreateProductReviewView(APIView):
    def post(self, request, productId, format=None):
        ...
        try:
            ...
            if Review.objects.filter(user=user, product=product).exists():
                return Response({'error': 'Review for this product already created'}, status=status.HTTP_409_CONFLICT)
...
class UpdateProductReviewView(APIView):
    def put(self, request, productId, format=None):
        ...
        try:
            ...
            return Response({'error': 'Rating ust be a decimal value'}, status=status.HTTP_404_NOT_FOUND)

        try:
            comment = str(data['comment'])
        except:
            return Response({'error': 'Must pass a comment when creating review'}, status=status.HTTP_404_NOT_FOUND)
    ...
    def delete(self, request, productId, format=None):
        ...
        try:
            ...
            if Review.objects.filter(user=user, product=product).exists():
                ...
                return Response({'error': 'Review for this product does not exits'}, status=status.HTTP_404_NOT_FOUND)
        except:
...
class FilterProductReviewsView(APIView):
    ...
    def get(self, request, productId, format=None):
        ...
        try:
            ...
            return Response({'error': 'Rating ust be a decimal value'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response({'error': 'Something went wrong when filtering product review'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
